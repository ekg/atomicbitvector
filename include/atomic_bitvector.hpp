...
public:
    ...
    struct it_state {
        size_t pos;
        inline void next(const atomic_bv_t* ref) { while (pos < ref->size() && ref->test(++pos)); }
        inline void prev(const atomic_bv_t* ref) { while (pos > 0 && ref->test(--pos)); }
        inline void begin(const atomic_bv_t* ref) { pos = 0; }
